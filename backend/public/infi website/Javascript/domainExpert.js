...
document.getElementById("form_Submit").addEventListener('click', async (e) => {
    ...
    const res_1 = await fetch("http://localhost:3000/domainExpert/insert", {
        method: "POST",
        headers: {
    ...
    if (res_1.status == 200) {
        const res_2 = await fetch("http://localhost:3000/domainExpert/upload_images", {
            method: "POST",
            body: formData
        })
        ...
        console.log(data_2)
        if (res_2.status == 200) {
            const res_2 = await fetch("http://localhost:3000/domainExpert/add_image", {
                method: "POST",
                headers: {
            'Content-Type':'application/json'
        },
